...
describe('nodeExecTask', () => {
  ...
  it('runs a script', async () => {
    const task = nodeExecTask({
      args: ['--max-old-space-size=4096', getMockScript('mock-success.js'), '--', 'arg1', 'arg2'],
    });
    ...
    expect(getNormalizedSpawnArgs(spawnSpy)).toEqual([
      ...
      `\${packageRoot}/src/__tests__/mock-success.js`,
      '--',
      'arg1',
      'arg2',
    ]);
  ...
  it('handles a script that fails', async () => {
    const task = nodeExecTask({
      args: [getMockScript('mock-fail.js')],
    });
    try {
  ...
  it('passes specified env only', async () => {
    const task = nodeExecTask({
      args: [getMockScript('mock-success.js')],
  ...
  it('errors if file not found', async () => {
    const script = path.join(__dirname, 'nope.js');
    const task = nodeExecTask({
      args: [script],
    });
    try {
